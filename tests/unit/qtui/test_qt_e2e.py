...
from sorter.qtui import settings_camera
from sorter.qtui.dialog_headstamps import HeadstampManagerDialog
...
def test_demo_a_full_sorting_session(config, window_factory, monkeypatch) -> None:
    ...
    # Counts survive Stop (Tk parity, jam-clearing): only an explicit reset
...
def test_demo_d_the_guide_dock_toggles_from_the_view_menu(window) -> None:
    toggles = {action.text(): action for action in window.menus["View"].actions()}
    assert set(toggles) == {"Serial Monitor", "Classification History", "User Guide Panel"}
