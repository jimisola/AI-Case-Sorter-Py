...
from sorter.qtui import settings_serial
from sorter.qtui.settings_serial import (
    AIRDROP_FIELDS,
    INIT_FIELDS,
...
def test_sort_to_slot_change_is_not_persisted(window, config) -> None:
    """Transient test control, not a saved setting — mirrors Tk's sort_to_var."""
    section = build_serial_section(window)
...
def test_init_on_startup_pushes_settings_after_connect(window, monkeypatch) -> None:
    """qtui's ``_after_connect`` (unlike the Tk one) doesn't push on its own —
...
def test_fetch_board_config_applies_matching_keys(window) -> None:
    ...
    # "slotquantity" matches nothing in _init_widgets (Tk's tab ignores it too).
    assert section.slot_count_spin.value() == 8
