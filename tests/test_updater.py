...
def test_tag_prefix_stripping_matches_the_publish_workflow(tag: str, expected: str) -> None:
    """The asset name is built from this on both sides -- publish.yml with
    ``${TAG#v}``, the client with _strip_tag_prefix -- and _pick_asset matches
    it *exactly*. Any disagreement means the client silently misses the real
    asset and falls back to the source archive, which carries no _version.py
    and so re-prompts forever (see test_apply_update.py).
    ...
    The expectations above were generated by running ``${TAG#v}`` for real.
    """
...
def _tar_bytes(entries: dict[str, str], *, symlink: tuple[str, str] | None = None) -> bytes:
    buf = io.BytesIO()
...
def test_stage_rejects_a_symlink_entry(monkeypatch) -> None:
    """Unlike a ZIP, a tarball can carry a symlink -- a real extraction
