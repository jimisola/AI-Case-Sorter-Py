...
launch. (``main.py --apply-update`` still invokes it too, as a compatibility
entry point for anything that launched the old way; ``bootstrap.py`` is the
live route.)
...
_VERSION_STAMP = "sorter/_version.py"
...
def _stamp_version(app_dir: Path, version: str, shipped: set[Path]) -> None:
    ...
    try:
        dest.parent.mkdir(parents=True, exist_ok=True)
        dest.write_text(
            '"""Written by sorter.apply_update — the applied release version.\n\n'
