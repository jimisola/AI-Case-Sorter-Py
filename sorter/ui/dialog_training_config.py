    ...
    def __init__(
        ...
        *,
        ...
        rows = [
            ...
            ("Learning rate", "learning_rate", "spin",
             {"from_": 1e-6, "to": 1.0, "increment": 1e-5, "format": "%.6f"}),
            ("Weight decay", "weight_decay", "spin",
             {"from_": 0.0, "to": 1.0, "increment": 1e-5, "format": "%.6f"}),
            ("Dropout", "dropout_rate", "spin",
             {"from_": 0.0, "to": 1.0, "increment": 0.05, "format": "%.2f"}),
            ("Validation split", "val_split", "spin",
             {"from_": 0.0, "to": 0.99, "increment": 0.05, "format": "%.2f"}),
            ("Image size", "image_size", "spin", {"from_": 64, "to": 512}),
            ("Max workers (-1 = auto)", "max_workers", "spin", {"from_": -1, "to": 64}),
            ("Stochastic depth prob (-1 = default)", "stochastic_depth_prob",
             "spin", {"from_": -1.0, "to": 0.5, "increment": 0.05, "format": "%.2f"}),
        ]
        ...
        flags_row = len(rows)
        ttk.Checkbutton(frm, text="Train on full dataset (override val split)",
                        variable=self.vars["train_all"]).grid(
            row=flags_row, column=0, columnspan=2, sticky="w", padx=4, pady=2,
        )
        ttk.Checkbutton(frm, text="Freeze backbone (only train classifier)",
                        variable=self.vars["freeze_backbone"]).grid(
            row=flags_row + 1, column=0, columnspan=2, sticky="w", padx=4, pady=2,
        )

        # Focal Loss
        ttk.Separator(frm, orient=tk.HORIZONTAL).grid(
            row=flags_row + 2, column=0, columnspan=2, sticky="ew", pady=8,
        )
        ttk.Checkbutton(frm, text="Use focal loss",
                        variable=self.vars["use_focal_loss"]).grid(
            row=flags_row + 3, column=0, sticky="w", padx=4, pady=2,
        )
        ttk.Spinbox(frm, from_=0.1, to=10.0, increment=0.1, format="%.1f", width=12,
                    textvariable=self.vars["focal_gamma"]).grid(
            row=flags_row + 3, column=1, sticky="w", padx=4, pady=2,
        )
        ttk.Label(frm, text="(focal gamma)").grid(row=flags_row + 4, column=1, sticky="w", padx=4)

        # SWA
        ttk.Separator(frm, orient=tk.HORIZONTAL).grid(
            row=flags_row + 5, column=0, columnspan=2, sticky="ew", pady=8,
        )
        ttk.Checkbutton(frm, text="Use stochastic weight averaging (SWA)",
                        variable=self.vars["use_swa"]).grid(
            row=flags_row + 6, column=0, columnspan=2, sticky="w", padx=4, pady=2,
        )
        ttk.Label(frm, text="SWA start (fraction of epochs)").grid(
            row=flags_row + 7, column=0, sticky="w", padx=4
        )
        ttk.Spinbox(frm, from_=0.0, to=1.0, increment=0.05, format="%.2f", width=12,
                    textvariable=self.vars["swa_start"]).grid(
            row=flags_row + 7, column=1, sticky="w", padx=4, pady=2,
        )
        ttk.Label(frm, text="SWA mode").grid(row=flags_row + 8, column=0, sticky="w", padx=4)
        ttk.Combobox(frm, values=["scheduled", "adaptive"], state="readonly",
                     textvariable=self.vars["swa_mode"], width=12).grid(
            row=flags_row + 8, column=1, sticky="w", padx=4, pady=2,
        )
