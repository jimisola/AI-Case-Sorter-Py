...
| artifact upload | Mechanism built and pushed (commit 9), `uv build` + `twine check --strict` verified locally against this real package. **Not yet exercised**: a real `release: published` event to confirm `gh release upload` actually attaches the wheel/sdist. |
| the sdist as the update payload, with a correct baked version | The hand-built app-archive `.zip` this row used to track is gone — the sdist replaces it (see the 2026-08-04 changelog entries). hatch-vcs's build hook against a real git-less copy of this repo was verified directly: confirmed it either crashes or silently overwrites a correct `sorter/_version.py`, which is why `bootstrap.py` runs with `--no-install-project`/`--no-sync`. `tests/test_sdist_contents.py` now builds the real sdist and asserts it carries the whole runtime file set plus `sorter/_version.py`, so the shape the updater depends on is checked rather than assumed. |
...
10. **`test_appenv.py` reaches into `test_community_api.py` by absolute module path** for
    two tests (`_api`, `_FakeResp`, `_FakeSession`), rather than either file owning them.
    Surfaced by the `tests/unit`/`tests/integration` split, which is what exposed it as a
    real cross-module coupling rather than two files that happened to sit side by side.
    A shared `tests/unit/_community_api_fakes.py` was drafted and confirmed to work
    (extract the four helpers, both files import from it, no behavior change), then
    reverted at the user's direction — "let's revert the reaching. that is another PR" —
    to keep the restructuring commit scoped to moving files, not restructuring their
    contents too. Worth doing as its own small follow-up.

11. **GitHub Release assets carry a `digest` field (`sha256:<hex>`), verified live against
    this repo's own uploaded assets via the API** — `sorter/updater.py`'s in-app updater
    could hash the downloaded sdist and compare before extracting, the same information
    already sitting in the same `/releases/latest` response it parses for tag/notes/assets
    today. Not implemented: this is a real security improvement (closes the "TLS is the
    entire trust anchor" gap noted elsewhere in this file), but a genuine design decision —
    what to do when `digest` is absent (GitHub's own schema says "string or null", not
    guaranteed) needs a real answer, not a quick addition, and deserves its own review
    rather than being folded into an already-large PR. `install-windows.ps1` was
    deliberately *not* given the same treatment in this PR — confirmed the pre-refactor
    zip installer had no checksum verification either, so shipping without it is parity,
    not a regression; this suggestion is about the in-app Python path specifically.
...
6. The updater has **no integrity or signature check beyond TLS**. `_pick_asset` now matches
   the sdist by exact name (the "first `.zip` asset" problem it used to have is gone), but
   nothing verifies the bytes: no checksum, no signature. The trust anchor is TLS to
   github.com plus whatever the release JSON says, and the archive is executed at next
   launch. The `_pick_asset` fallback to `archive/refs/tags/<tag>.tar.gz` is a further
   step down, since a tag can be moved or recreated without anything user-visible changing.
   Publishing a `SHA256SUMS` asset (or signing) and verifying before extraction is the fix;
   see also the Windows installer, which downloads the same artifact with the same
   guarantees.
...
| 2026-08-04 | A real 0.1.0 release surfaced two genuine defects: `installer/install-windows.bat`'s committed blob was never renormalized after `.gitattributes` landed, so every checkout looked "dirty" to git and hatch-vcs baked `.devN+dirty` into the built version instead of the real tag (fixed by renormalizing); and `publish.yml`/`updater.py` had no check that the built version actually matched the release tag (fixed by an assertion in `publish.yml`). That in turn prompted replacing the hand-built app-archive `.zip` entirely: the project's sdist already carries the correct version (hatch-vcs's build hook stamps it into every build target, not just the wheel), so `sorter/updater.py` now downloads that directly and `publish.yml`'s separate archive-build step is gone. One build path instead of two, so the class of bug that started this can't recur. `sorter/updater.py`'s extraction logic moved from `zipfile` to `tarfile`, which needed a genuinely new check `zipfile` never had to think about: a tarball can carry symlinks, and one pointing outside the extraction directory is a real attack class a ZIP can't even express -- every non-regular-file entry is now rejected outright. |
| 2026-08-04 | A review pass over the zip→sdist change found two real defects and a set of smaller ones. **The Windows installer was broken by the switch, in a file the change never touched**: `install-windows.ps1` picked the first `*.zip` release asset, and after the switch releases carry none — so every fresh install silently fell through to GitHub's source zip, which has no `sorter/_version.py`. That reports `0.0.0+unknown`, which parses as a pre-release, so the update prompt would return on every launch forever. The installer now downloads the sdist and extracts it with `tar.exe`. Worth recording as a pattern, not just a bug: diff-scoped review would never have found it, because the diff was correct and the breakage was in what the diff *implied* elsewhere. **Second: `_safe_members`' absolute-path check was too narrow on Windows** — it only inspected position 1 of the whole entry name, so a drive letter in a later component (`pkg/D:/evil.py`) passed, and `Path()` re-anchors on it, resolving outside the staging directory entirely and potentially into the live app folder. Pre-existing (the `zipfile` version had it too), but this change rewrote and re-justified exactly that block as the security story for the format switch, so it was fixed here: per-component rejection plus a resolved-path containment check against the staging root, which is what `tarfile`'s own `data` filter does. Also: `MAX_ARCHIVE_BYTES` counts declared payload bytes and so never bounded an archive of *empty* entries (added `MAX_ARCHIVE_ENTRIES`, and switched to streaming members rather than `getmembers()`, which decompressed the whole archive before any check could fire); `is_tarfile()` accepted tars that `mode="r:gz"` then rejected, bypassing the friendly error; the release tag is now validated before being interpolated into a URL; downloads are refused over anything but HTTPS, re-checked after redirects. `tests/test_sdist_contents.py` is new and is the one that actually guards the change's premise — it builds the real sdist and asserts the runtime file set, since nothing else declares what the sdist must contain. |
| 2026-08-04 | Split `tests/` into `tests/unit/` and `tests/integration/` (idea from reqstool-client/tests), via `git mv` so history/blame follow each file. Fixed five broken `Path(__file__).resolve().parent.parent` repo-root computations and every hardcoded path reference (`lint.yml`, two ruff per-file-ignores, two doc mentions) the move touched. Found and fixed a real cross-module import (`test_appenv.py` reaching into `test_community_api.py` by absolute path) that the move exposed. Drafted a shared-fixture extraction for that coupling, then reverted it per direction to keep this commit scoped — recorded as suggestion §9.10 for a follow-up PR. |
| 2026-08-05 | Fixed two real gaps in `install-windows.ps1` beyond the review's scope: `-Version` bypassed the sdist-matching logic entirely, hardcoding the source zip and reintroducing the exact `0.0.0+unknown` bug that path was just fixed for; and raw `tar.exe` extraction had no entry-level sanitization, unlike the Python side. Verified the drive-letter concern is real, not theoretical: extracted a real bsdtar 3.7.7 locally (no root needed, via `dpkg-deb -x`) and confirmed its own extraction rejects a `..` traversal entry but does **not** reject `pkg/D:/evil.py` — it just writes a literal `D:` folder on Linux, where Windows' path-join could instead read it as a drive reference. Added name-based pre-extraction checks (absolute path, drive letter, traversal) rather than trust unverified behavior on the one platform this can't be tested against. Declined sha256 asset-digest verification for this PR — confirmed the pre-refactor zip installer had none either (`git show` of the prior revision), so omitting it is genuine parity, not a regression; recorded as a follow-up. Added a `-Repo` override parameter (mirrors `sorter/updater.py`'s `CASESORTER_UPDATE_REPO`) and a new `installer-smoke.yml` workflow that actually runs the installer on `windows-latest` against a real release — path-filtered to `installer/**` plus `release: published`, deliberately using `shell: powershell` (5.1) rather than `pwsh`, since 5.1 is what the script's own ASCII-only comment is hardened against and what a real double-click actually invokes. This closes a real gap: no CI job had ever executed this script before, only statically checked it (ASCII/CRLF/quote-balance). |
