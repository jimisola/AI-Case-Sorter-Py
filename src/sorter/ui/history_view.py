...
one ``RunController`` posts and ``ui.app._on_run_history`` reads for the Sort
...
class HistoryView(QWidget):
    ...
    Semantics deliberately match the Windows app's Monitor: tiles never move
    or scroll — a new record overwrites the oldest cell in place, and the
    recency snake is what points at "current".
