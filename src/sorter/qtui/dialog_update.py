"""Software update — review, download, restart. Qt half of ``ui/dialog_update.py``.
...
Two differences from the Tk dialog, both from JL's decision to put updates under
**Help → "Check for updates…"** rather than in Settings:
...
* ``check_on_open=True`` lets the menu action open the dialog and run the check
  *inside* it, so the user gets a window immediately instead of a status-bar
  message. Passing a ``info=`` the shell already has (the silent startup check)
  still works and skips the check entirely — that is the Tk path.
* A check that fails is shown here rather than in the status bar; the dialog is
  the only surface the menu action opens.

Threading follows CLAUDE.md §8 and the Tk dialog's shape: a worker only ever
puts a message on a ``queue.Queue`` and a main-thread ``QTimer`` drains it into
the widgets. Two independent queue/timer pairs, for the same reason Tk has two —
a release-list load and a download can be in flight at once, and one queue would
force the consumer to disentangle them by payload shape. The initial check rides
the metadata pair (it can't overlap a picker load: both go through
``_loading_meta``).

Release notes are Markdown; ``QTextBrowser.setMarkdown`` renders them, so the Tk
side's hand-rolled tokenizer has no Qt counterpart. ``linkify_notes`` supplies
the two things GitHub does that CommonMark doesn't — ``@user`` and ``#123`` —
plus the same size caps, and links open through a scheme check rather than
``setOpenExternalLinks``: a release body is attacker-influenced in principle
(``CASESORTER_UPDATE_REPO`` points wherever it points).
"""
...
# rather than wedge the UI laying out megabytes of text. Same caps as
# ui/markdown_render.py.
