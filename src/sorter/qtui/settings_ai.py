...
class AiSection(QWidget):
    ...
    def clear_all(self) -> None:
        # Tk confirms here too — this deletes every headstamp in one click.
    ...
    def run_test(self) -> None:
        ...
        The Tk tab feeds a case through the board first (``RunController.test_once``);
        this page only needs a frame, so a board isn't required to check the
        server settings. Everything the worker reads is snapshotted here — it
        must not touch widgets or the DB-backed Config.
        """
        values = self._field_values()
