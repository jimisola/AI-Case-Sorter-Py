"""Per-model training hyperparameters — the Qt half of ``ui/dialog_training_config.py``.

Same fields, same ranges, and the same round-trip: the dialog edits the
``TrainingConfig`` instance it was handed (in place, as Tk does) and calls
``on_saved`` with it, so the caller only has to persist the model row.

The three SWA tuning values (``swa_acc_threshold``, ``swa_patience``,
``swa_min_epoch``) get real widgets here. Tk builds Tk variables for them and
writes them straight back unedited — every one of them reaches the trainer's
argv, so they were exposed in everything but the layout.
"""
...
# (key, label, minimum, maximum, step, decimals) — decimals 0 means an int
# field. Ranges are the Tk dialog's, verbatim.
...
class TrainingConfigDialog(QDialog):
    ...
    def save(self) -> None:
        ...
        here — unlike Tk, where a Spinbox's text can be anything the user typed.
        Types come from the field spec, not from what the attribute happens to
