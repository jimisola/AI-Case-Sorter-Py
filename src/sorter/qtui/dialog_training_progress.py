"""Live training console — the Qt half of ``ui/dialog_training_progress.py``.
...
Two things it does that Tk's doesn't:

* **It unsubscribes on close.** Tk's leaves its handlers on the bus and guards
  each one with ``winfo_exists``; a Qt widget that has been deleted raises
  ``RuntimeError`` from anywhere in the C++ half, so the handlers come off.
